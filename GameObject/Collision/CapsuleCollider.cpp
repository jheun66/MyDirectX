...
bool CapsuleCollider::IsSphereCollision(SphereCollider* collider)
{
    ...
    float distance = GameMath::Distance(pointOnLine, collider->WorldPos());

    return distance <= (radius + collider->GetRadius());
}
...
bool CapsuleCollider::IsCapsuleCollision(CapsuleCollider* collider)
{
    ...
    Vector3 bestB = GameMath::ClosestPointOnLineSegment(bA, bB, bestA);
