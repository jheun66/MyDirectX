...
void Breath::Create()
{
	...
	{
		vertices[i].size.x = GameMath::Random(minSize.x, maxSize.x);
		vertices[i].size.y = GameMath::Random(minSize.y, maxSize.y);

		float radius = GameMath::Random(minSpeed, maxSpeed);

		Vector3 velocity = Vector3(0, 0, radius);

		XMFLOAT3 rot;
		rot.x = GameMath::Random(0.0f, XM_PIDIV2);
		rot.y = GameMath::Random(0.0f, XM_PIDIV2);
		rot.z = GameMath::Random(0.0f, XM_PIDIV2);
		...
		vertices[i].startTime = GameMath::Random(0.0f, maxStartTime);
	}
...
void Breath::Update()
{
	...
	breathBuffer->data.time += Time::Delta();
