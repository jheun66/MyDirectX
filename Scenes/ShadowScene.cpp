...
void ShadowScene::SetViewProjection()
{
	...
	XMMATRIX rotMat = XMMatrixRotationRollPitchYaw(rotX, rotY, 0.0f) * XMMatrixRotationX(-XM_PIDIV2);

	XMVECTOR up = XMVector3TransformNormal(GameMath::kUp, rotMat);

	XMMATRIX view = XMMatrixLookAtLH(lightPos.data, XMVectorZero(), up);
	XMMATRIX projection = XMMatrixOrthographicLH(radius * 2, radius * 2, -radius, radius);
